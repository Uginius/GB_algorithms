...
public class BreadthFirstSearch {
    ...
    public BreadthFirstSearch(Graph g, int source) {
        if (source < 0 || source > g.vertexCount() - 1) throw new IndexOutOfBoundsException("Wrong vertex number.");
        this.source = source;
        edgeTo = new int[g.vertexCount()];
        marked = new boolean[g.vertexCount()];
        distTo = new int[g.vertexCount()];
        for (int i = 0; i < g.vertexCount(); i++) distTo[i] = inf;
        bfs(g, source);
    }
    ...
    public boolean hasPathTo(int dist) {
        if (dist < 0 || dist > marked.length - 1) throw new IndexOutOfBoundsException("Wrong vertex number.");
        return marked[dist];
    }
